...
class XingeApp {
	...
	/**
	 * 使用默认设置推送通知给单个/多个/all android版账户
	 * $type=1 通知，一般app没有前台打开 type=2已打开
	 */
	public static function PushAccountAndroid($accessId, $secretKey, $title, $content, $account, $type=0)
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new Message();
		$mess->setTitle($title);
		$mess->setContent($content);
		if($type==0)$type = Message::TYPE_NOTIFICATION;
		$mess->setType($type);
		...
		$mess->setAction($action);
		if(is_array($account)){
			$ret = $push->PushAccountList(0, $account, $mess);
		}else{
			if($account=='all'){
