...
$(document).ready(function(){
	var c={
		yulan:function(){
			...
			s+='<tr><td></td><td>名称</td><td>分类</td><td>单价</td><td>单位</td><td>规格</td><td>型号</td></tr>';
			a = cont.split('\n');
		...
		downss:function(){
			js.open('upload/base/goodsmport.xls');
		},
		insrtss:function(){
			var val = mobjs.val();
			mobjs.val(val+'	');
		...
		saveadd:function(){
			...
			js.setmsg('处理中...','', vis);
			js.ajax(js.getajaxurl('addplgoods','{mode}','{dir}'),{importcont:val},function(ds){
				if(ds.success){
					js.msg('success', ds.msg);
					closenowtabs();
					try{goodsmanagesss.reload();}catch(e){}
				}else{
...
<div align="left">
<div>请根据Excel格式添加数据，分类格式如：[办公耗材/鼠标]，将数据复制到下面文本框中，<a click="downss" href="javascript:;">下载导入Excel表格格式</a><br>也可以手动输入，多行代表多记录，整行字段用	分开，<a click="insrtss" href="javascript:;">插入间隔符</a></div>
